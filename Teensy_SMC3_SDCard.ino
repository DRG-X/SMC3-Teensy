...
// #include <SoftwareSerial.h> removing AVR garbage 
...
// // #endif    
    ...
    // analogWrite(PWMpin1,0);      //analogWrite(PWMpin1, 0);
    // analogWrite(PWMpin2,0);      //analogWrite(PWMpin2, 0);
...
void applyConfigLine(String line) {
  ...
  int ivalue = val.toInt();

  if (key == "Kp1") Kp1_x100 = ivalue;
  else if (key == "Ki1") Ki1_x100 = ivalue;
  else if (key == "Kd1") Kd1_x100 = ivalue;

  else if (key == "Kp2") Kp2_x100 = ivalue;
  else if (key == "Ki2") Ki2_x100 = ivalue;
  else if (key == "Kd2") Kd2_x100 = ivalue;

  else if (key == "Kp3") Kp3_x100 = ivalue;
  else if (key == "Ki3") Ki3_x100 = ivalue;
  else if (key == "Kd3") Kd3_x100 = ivalue;

  else if (key == "Ks1") Ks1 = ivalue;
  else if (key == "Ks2") Ks2 = ivalue;
  else if (key == "Ks3") Ks3 = ivalue;


  else if (key == "PWMmax1") PWMmax1 = ivalue;
  else if (key == "PWMmax2") PWMmax2 = ivalue;
  else if (key == "PWMmax3") PWMmax3 = ivalue;

  else if (key == "DeadZone1") DeadZone1 = ivalue;
  else if (key == "DeadZone2") DeadZone2 = ivalue;
  else if (key == "DeadZone3") DeadZone3 = ivalue;

  else if (key == "PWMoffset1") PWMoffset1 = ivalue;
  else if (key == "PWMoffset2") PWMoffset2 = ivalue;
else if (key == "PWMoffset3") PWMoffset3 = ivalue;

else if (key == "PWMrev1") PWMrev1 = ivalue;
else if (key == "PWMrev2") PWMrev2 = ivalue;
else if (key == "PWMrev3") PWMrev3 = ivalue;

else if (key == "CutoffLimitMin1") CutoffLimitMin1 = ivalue;
else if (key == "CutoffLimitMin2") CutoffLimitMin2 = ivalue;
else if (key == "CutoffLimitMin3") CutoffLimitMin3 = ivalue;

else if (key == "InputClipMin1") InputClipMin1 = ivalue;
else if (key == "InputClipMin2") InputClipMin2 = ivalue;
else if (key == "InputClipMin3") InputClipMin3 = ivalue;

else if (key == "PIDProcessDivider") PIDProcessDivider = constrain(ivalue, 1, 10);
else if (key == "Timer1FreqkHz") Timer1FreqkHz = ivalue;
else if (key == "Timer2FreqkHz") Timer2FreqkHz = ivalue;
...
void EnsureSDConfigExists() {
  if (!SD.begin(BUILTIN_SDCARD)) {
    Serial.println("⚠️ SD init failed (ensure), using defaults");
    return;
  }

  if (SD.exists("smc3.cfg")) {
    // already exists, good
    return;
  }

  Serial.println("⚠️ smc3.cfg missing → creating default config");

  // Create using whatever defaults are currently in variables
  SaveSDConfig();
}
...
void ReadSDConfig() {
  if (!SD.begin(BUILTIN_SDCARD)) {
    Serial.println(" SD init failed, using defaults");
    return;
  }

 
  EnsureSDConfigExists();

  File file = SD.open("smc3.cfg");
  if (!file) {
    Serial.println("⚠️ smc3.cfg not found even after ensure, using defaults");
    return;
  }
  ...
  while (file.available()) {
    ...
    if (c == '\n') {
      line.trim();
      if (line.length() > 0) applyConfigLine(line);
      line = "";
    } else {
      line += c;
    }
  ...
  analogWriteFrequency(PWMpin1, Timer1FreqkHz * 1000);
  analogWriteFrequency(PWMpin2, Timer1FreqkHz * 1000);
  analogWriteFrequency(PWMpin3, Timer2FreqkHz * 1000);
...
void sanitizeParams() {
  // --- PID gains (x100 scaling already)
  Kp1_x100 = clampValue<long>(Kp1_x100, 0, 2000);
  Ki1_x100 = clampValue<long>(Ki1_x100, 0, 2000);
  Kd1_x100 = clampValue<long>(Kd1_x100, 0, 2000);

  Kp2_x100 = clampValue<long>(Kp2_x100, 0, 2000);
  Ki2_x100 = clampValue<long>(Ki2_x100, 0, 2000);
  Kd2_x100 = clampValue<long>(Kd2_x100, 0, 2000);

  Kp3_x100 = clampValue<long>(Kp3_x100, 0, 2000);
  Ki3_x100 = clampValue<long>(Ki3_x100, 0, 2000);
  Kd3_x100 = clampValue<long>(Kd3_x100, 0, 2000);

  // --- Ks filtering (must be >=1)
  Ks1 = clampValue<int>(Ks1, 1, 50);
  Ks2 = clampValue<int>(Ks2, 1, 50);
  Ks3 = clampValue<int>(Ks3, 1, 50);

  // --- PWM limits
  PWMoffset1 = clampValue<int>(PWMoffset1, 0, 255);
  PWMoffset2 = clampValue<int>(PWMoffset2, 0, 255);
  PWMoffset3 = clampValue<int>(PWMoffset3, 0, 255);

  PWMmax1 = clampValue<int>(PWMmax1, 0, 255);
  PWMmax2 = clampValue<int>(PWMmax2, 0, 255);
  PWMmax3 = clampValue<int>(PWMmax3, 0, 255);

  PWMrev1 = clampValue<int>(PWMrev1, 0, 255);
  PWMrev2 = clampValue<int>(PWMrev2, 0, 255);
  PWMrev3 = clampValue<int>(PWMrev3, 0, 255);

  // PWMoffset should not exceed PWMmax
  if (PWMoffset1 > PWMmax1) PWMoffset1 = PWMmax1;
  if (PWMoffset2 > PWMmax2) PWMoffset2 = PWMmax2;
  if (PWMoffset3 > PWMmax3) PWMoffset3 = PWMmax3;

  // --- Deadzone reasonable
  DeadZone1 = clampValue<int>(DeadZone1, 0, 50);
  DeadZone2 = clampValue<int>(DeadZone2, 0, 50);
  DeadZone3 = clampValue<int>(DeadZone3, 0, 50);

  // --- Cutoff & clip mins (0..1023)
  CutoffLimitMin1 = clampValue<int>(CutoffLimitMin1, 0, 1023);
  CutoffLimitMin2 = clampValue<int>(CutoffLimitMin2, 0, 1023);
  CutoffLimitMin3 = clampValue<int>(CutoffLimitMin3, 0, 1023);

  InputClipMin1 = clampValue<int>(InputClipMin1, 0, 1023);
  InputClipMin2 = clampValue<int>(InputClipMin2, 0, 1023);
  InputClipMin3 = clampValue<int>(InputClipMin3, 0, 1023);

  // Recalculate maxes the way original SMC3 expects
  ...
  // --- PID divider
  PIDProcessDivider = clampValue<int>(PIDProcessDivider, 1, 10);

  // --- PWM freq kHz constraints (basic safe)
  Timer1FreqkHz = clampValue<unsigned int>(Timer1FreqkHz, 1, 50);
  Timer2FreqkHz = clampValue<unsigned int>(Timer2FreqkHz, 1, 50);
}
...
void SaveSDConfig() {
    sanitizeParams();
  if (!SD.begin(BUILTIN_SDCARD)) {
    Serial.println("❌ SD init failed (save)");
    return;
  }

  File file = SD.open("smc3.cfg", FILE_WRITE);
  if (!file) {
    Serial.println("❌ Could not open smc3.cfg for writing");
    return;
  }

  // ✅ truncate old file (important!)
  file.seek(0);
  file.truncate(0);

  // ---- WRITE CONFIG ----
  file.println("Kp1=" + String(Kp1_x100));
  file.println("Ki1=" + String(Ki1_x100));
  file.println("Kd1=" + String(Kd1_x100));

  file.println("Kp2=" + String(Kp2_x100));
  file.println("Ki2=" + String(Ki2_x100));
  file.println("Kd2=" + String(Kd2_x100));

  file.println("Kp3=" + String(Kp3_x100));
  file.println("Ki3=" + String(Ki3_x100));
  file.println("Kd3=" + String(Kd3_x100));

  file.println("Ks1=" + String(Ks1));
  file.println("Ks2=" + String(Ks2));
  file.println("Ks3=" + String(Ks3));

  file.println("PWMmax1=" + String(PWMmax1));
  file.println("PWMmax2=" + String(PWMmax2));
  file.println("PWMmax3=" + String(PWMmax3));

  file.println("DeadZone1=" + String(DeadZone1));
  file.println("DeadZone2=" + String(DeadZone2));
  file.println("DeadZone3=" + String(DeadZone3));

  file.println("PWMoffset1=" + String(PWMoffset1));
  file.println("PWMoffset2=" + String(PWMoffset2));
  file.println("PWMoffset3=" + String(PWMoffset3));

  file.println("PWMrev1=" + String(PWMrev1));
  file.println("PWMrev2=" + String(PWMrev2));
  file.println("PWMrev3=" + String(PWMrev3));

  file.println("CutoffLimitMin1=" + String(CutoffLimitMin1));
  file.println("CutoffLimitMin2=" + String(CutoffLimitMin2));
  file.println("CutoffLimitMin3=" + String(CutoffLimitMin3));

  file.println("InputClipMin1=" + String(InputClipMin1));
  file.println("InputClipMin2=" + String(InputClipMin2));
  file.println("InputClipMin3=" + String(InputClipMin3));

  file.println("PIDProcessDivider=" + String(PIDProcessDivider));
  file.println("Timer1FreqkHz=" + String(Timer1FreqkHz));
  file.println("Timer2FreqkHz=" + String(Timer2FreqkHz));

  file.close();
  Serial.println("✅ Saved config to SD (smc3.cfg)");
}
...
//                    default:
        ...
        case 'e':
            if (RxBuffer[1][ComPort]=='n' && RxBuffer[2][ComPort]=='a')   // Enable all motors
            {
                ...
                digitalWriteFast(ENBpin1, LOW);
                digitalWriteFast(ENBpin2, LOW);
                digitalWriteFast(ENBpin3, LOW);
                // PORTD = OutputPort;
                digitalWriteFast(ENBpin1, HIGH);
                digitalWriteFast(ENBpin2, HIGH);
                digitalWriteFast(ENBpin3, HIGH);
                // PORTD = OutputPort;
#endif
            }
...
#ifdef MODE2    // 43A "Chinese" H-Bridge
                // Reset any overtemp shutdowns and enable the H-Bridges  - toggle pins low then leave high
                digitalWriteFast(ENBpin1, LOW);      // Unset Motor1 In 2
                // PORTD = OutputPort;
                digitalWriteFast(ENBpin1, HIGH);          // Set Motor1 In 2
                // PORTD = OutputPort;
#endif
            }
...
#ifdef MODE2    // 43A "Chinese" H-Bridge
                // Reset any overtemp shutdowns and enable the H-Bridges  - toggle pins low then leave high
                digitalWriteFast(ENBpin2, LOW);      // Unset Motor2 In 2
                // PORTD = OutputPort;
                digitalWriteFast(ENBpin2, HIGH);;           // Set Motor2 In 2
                // PORTD = OutputPort;
#endif
            }
...
#ifdef MODE2    // 43A "Chinese" H-Bridge
                // Reset any overtemp shutdowns and enable the H-Bridges  - toggle pins low then leave high
                digitalWriteFast(ENBpin3, LOW);        // Unset Motor3 In 2
                // PORTD = OutputPort;
                digitalWriteFast(ENBpin3, HIGH);           // Set Motor3 In 2
                // PORTD = OutputPort;
#endif
            }
            break;
...
void SetOutputsMotor1()
{
    ...
    {
        PWMout1 = PWMrev1;
        digitalWriteFast(ENApin1, LOW);   // Unset Motor1 In 1
        digitalWriteFast(ENBpin1, HIGH);           // Set Motor1 In 2
    ...
    {
        PWMout1 = PWMrev1;
        digitalWriteFast(ENApin1, HIGH);    // Set Motor1 In 1
        digitalWriteFast(ENBpin1, LOW);        // Unset Motor1 In 2
        analogWrite(PWMpin1, PWMrev1);
    }  
...
void SetOutputsMotor2()
{
    ...
    {
        PWMout2 = PWMrev2;
        digitalWriteFast(ENApin2, LOW);  // Unset Motor1 In 1
        digitalWriteFast(ENBpin2, HIGH);           // Set Motor1 In 2
        analogWrite(PWMpin2, PWMrev2);
    }
    ...
    else if(Target2 > (Feedback2 + DeadZone2) || Target2 < (Feedback2 - DeadZone2))
    {
        ...
        {                                              
            ...
            digitalWriteFast(ENApin2, LOW);        // Unset Motor2 In 1
            digitalWriteFast(ENBpin2, HIGH);           // Set Motor2 In 2
        }
        analogWrite(PWMpin2, PWMout2);
    }
    else
    {
        // Brake Motor 
...
void SetOutputsMotor3()
{
    ...
    {
        PWMout3 = PWMrev3;
        digitalWriteFast(ENApin3, LOW);  // Unset Motor1 In 1
        digitalWriteFast(ENBpin3, HIGH);           // Set Motor1 In 2
    ...
    else if(Target3 > (Feedback3 + DeadZone3) || Target3 < (Feedback3 - DeadZone3))
    {
        ...
        {                                              
            ...
            PWMout3+=PWMoffset3;
            if(PWMout3> PWMmax3){PWMout3=PWMmax3;}
            digitalWriteFast(ENApin3, LOW);        // Unset Motor3 In 1
            digitalWriteFast(ENBpin3, HIGH);           // Set Motor3 In 2
        }
...
void SetOutputsMotor1()   // MODE2
{
    ...
    else if((Feedback1<InputClipMin1) && (PWMrev1 != 0))
    {
       ...
       digitalWriteFast(ENApin1, HIGH);
       analogWrite(PWMpin1, 255-PWMrev1);
    }  
    else if((Target1 > (Feedback1 + DeadZone1)) || (Target1 < (Feedback1 - DeadZone1)))
    {
        if (PWMout1 >= 0)  
        {                                    
            ...
            if(PWMout1 > (PWMmax1+LiftFactor1)){PWMout1=PWMmax1+LiftFactor1;}
            digitalWriteFast(ENApin1, HIGH) ;          // Set Motor1 In 1
            analogWrite(PWMpin1, 255-PWMout1);    // Motor driven when PWM=0 (unset)
        }  
        else 
        {                                              
            // Drive Motor Backwards 
            ...
            if(PWMout1 > PWMmax1){PWMout1=PWMmax1;}
            digitalWriteFast(ENApin1, LOW);        // Unset Motor1 In 1
            analogWrite(PWMpin1, PWMout1);        // Motor driven when PWM=1 (set)
        }
    }
    else
    {
...
void SetOutputsMotor2()   // MODE2
{
    ...
    {
       PWMout2 = PWMrev2;
       digitalWriteFast(ENApin2, HIGH);    // Set Motor2 In 1
       analogWrite(PWMpin2, 255-PWMrev2);
    }  
    else if((Target2 > (Feedback2 + DeadZone2)) || (Target2 < (Feedback2 - DeadZone2)))
    {
        if (PWMout2 >= 0)  
        {                                    
            ...
            // OutputPort |= 1 << ENApin2;           // Set Motor2 In 1
            digitalWriteFast(ENApin2, HIGH);
            analogWrite(PWMpin2, 255-PWMout2);
        }  
        else 
        {                                              
            // Drive Motor Backwards
...
void setupPWM() {
  analogWriteResolution(8);      // SMC3 expects 0–255
  analogWriteFrequency(PWMpin1, 25000);  // Motor 1
  analogWriteFrequency(PWMpin2, 25000);  // Motor 2
  analogWriteFrequency(PWMpin3, 31000);  // Motor 3
}